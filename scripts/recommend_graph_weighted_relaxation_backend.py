...
def write_report(rows: list[dict[str, object]], output_dir: Path) -> Path:
    ...
    lines.append("")
    lines.append("For this benchmark, CPU Dijkstra remains the best backend for high-diameter chain/grid/layered cases in the measured range. The global GPU relaxation becomes useful on random graphs where enough edge parallelism exists and the iteration count remains low. The current frontier GPU variant is experimental and usually loses because per-iteration frontier management and duplicate frontier entries dominate.")
    lines.append("")
    ...
    lines.append("")
    lines.append("The next algorithmic step should not be another small frontier tweak. It should be a bucketed shortest-path method such as delta-stepping, or a structure-aware backend: prefix/scan-style treatment for chain-like graphs, tiled/local methods for grids, and global GPU scans for low-diameter random graphs.")
...
def write_plot(rows: list[dict[str, object]], output_dir: Path) -> Path | None:
    ...
    backend_index = {"cpu": 0, "gpu-global": 1, "gpu-frontier": 2}
    backend_label = {0: "CPU", 1: "GPU global", 2: "GPU frontier"}
    ...
    if not plotted:
        ...
        return None
    ax.set_xscale("log")
    ax.set_yticks([0, 1, 2], [backend_label[0], backend_label[1], backend_label[2]])
    ax.set_xlabel("nodes")
